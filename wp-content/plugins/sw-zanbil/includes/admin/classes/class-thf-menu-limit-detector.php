...
if( ! class_exists( 'THF_MENU_LIMIT_DETECTOR' )){

	class THF_MENU_LIMIT_DETECTOR{
		...
		function check_post_limits(){
			...
			$current_post_vars_count = get_option( 'thf-post-var-count', 0 );
			...
			if( $r['suhosin_post_maxvars'] != '' || $r['suhosin_request_maxvars'] != '' || $r['max_input_vars'] != '' ){
				...
				if( ( $r['suhosin_post_maxvars'] != ''    && $r['suhosin_post_maxvars']    < 3000 ) ||
					...
					$message[] = esc_html__( 'Your server is running Suhosin, and your current maxvars settings may limit the number of menu items you can save.', TCW_TEXTDOMAIN );
				}
		...
		function menus_save_post_vars( $menu_id ){
			...
			if( ! empty( $locations['primary'] ) && $menu_id == $locations['primary'] ){
				if( isset( $_POST['save_menu'] ) ){
					...
					update_option( 'thf-post-var-count', $count );
				}
			}
		}
		...
		function display_message( $message ){
			...
			if( ! empty( $message ) ){
				...
				<div class="error notice">
					<h4><?php esc_html_e( 'Menu Item Limit Warning', TCW_TEXTDOMAIN ); ?></h4>
					<ul>
						<?php
