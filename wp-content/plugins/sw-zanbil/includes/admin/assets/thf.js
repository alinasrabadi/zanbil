...
$window.load(function () {
    jQuery('.widgets-chooser-sidebars').find('li').each(function () {
        ...
        if (/^Page #\d+ - Section #\d+$/.test(thistext)) {
            $thisElem.addClass('thf-chooser-section').hide();
        }
    });
});
...
$doc.ready(function () {
    ...
    var $thfBody = jQuery('body');


    /* DASHBORED COLOR
	------------------------------------------------------------------------------------------ */
    var brandColor = '#d54e21';
    if ($thfBody.hasClass('admin-color-blue')) {
        brandColor = '#e1a948';
    }
    else if ($thfBody.hasClass('admin-color-coffee')) {
        brandColor = '#9ea476';
    }
    else if ($thfBody.hasClass('admin-color-ectoplasm')) {
        brandColor = '#d46f15';
    }
    else if ($thfBody.hasClass('admin-color-midnight')) {
        brandColor = '#69a8bb';
    }
    else if ($thfBody.hasClass('admin-color-ocean')) {
        brandColor = '#aa9d88';
    }
    else if ($thfBody.hasClass('admin-color-sunrise')) {
        brandColor = '#ccaf0b';
    }


    jQuery('.thf-toggle-option').each(function () {
        var $thisElement = jQuery(this),
            elementType  = $thisElement.attr('type'),
            toggleItems  = $thisElement.data('thf-toggle');
	...
	------------------------------------------------------------------------------------------ */
    jQuery('#revoke-thf-token').click(function () {
	...
	------------------------------------------------------------------------------------------ */
    jQuery('#thf-reset-settings').click(function () {
        var message = jQuery(this).data('message'),
	...
	------------------------------------------------------------------------------------------ */
    jQuery('#thf-import-upload').click(function () {

        var importSettings = jQuery('#thf-import-file').val();
    ...
    jQuery('#send-notification').click(function () {
        ...
        var $theParent   = jQuery('#foxpush-create-campaign'),
            ...
            theTitle     = jQuery('#thf_foxpush_title').val(),
            theMessage   = jQuery('#thf_foxpush_msg').val(),
            theImage     = jQuery('#thf_foxpush_icon').val(),
        ...
        jQuery.ajax({
            ...
            data: {
                ...
                image  : theImage,
                id     : thePostID,
                action : 'thf-foxpush-send-campaign',
            },
    ...
    jQuery('#update-campaign-status').click(function () {
        ...
        jQuery.ajax({
            ...
            data: {
                id    : thePostID,
                action: 'thf-foxpush-show-campaign',
            },
	...
	------------------------------------------------------------------------------------------ */
    if (jQuery('.widgets-php div[id*="thfpost-"]').length) {
        jQuery('#thf-show-sections-sidebars-wrap').show();

        jQuery('.widgets-php div[id*="thfpost-"]').parent().addClass('thf-sidebar-section');
        jQuery('#thf-show-sections-sidebars').change(function () {
            jQuery('.thf-sidebar-section, .thf-chooser-section').toggle();
        });
    }
    ...
    $searchTheme.on('keyup', function () {
        ...
        if (valThis == '') {
            ...
            jQuery('.thf-label').each(function () {
                var $thisElem = jQuery(this),
                    thistext  = $thisElem.text();
                ...
                if (thistext.toLowerCase().indexOf(valThis) >= 0) {
                    ...
                    var thistextid       = $thisElem.closest('.option-item').attr('id'),
                        ...
                        thistextparent   = $thisparent.find('.thf-tab-head h2').text(),
    ...
    $searchList.on('click', 'a', function () {
        ...
        jQuery('a[href="#' + tabId + '"]').get(0).click();
    ...
    $doc.mouseup(function (e) {
        var container = jQuery('#plugin-options-search-wrap');
    ...
    /* LIVE HEADER PREVIEW
	------------------------------------------------------------------------------------------ */
    if ($thfBody.hasClass('toplevel_page_thf-plugin-options')) {
        //Chnage the header layout ----------
        var selected_val           = jQuery("input[name='thf_options[header_layout]']:checked").val(),
        ...
        jQuery("input[name='thf_options[header_layout]']").change(function () {
        ...
        jQuery("input[name='thf_options[top_nav]']").change(function () {
            $header_preview_area.toggleClass('top-nav-disabled');
        });

        jQuery("input[name='thf_options[top_nav_position]']").change(function () {
            $header_preview_area.toggleClass('top-nav-below');
        });

        jQuery("input[name='thf_options[top_nav_layout]']").change(function () {
            jQuery('.top-bar-wrap').toggleClass('top-nav-full');
        });

        jQuery("input[name='thf_options[top_nav_dark]']").change(function () {
            jQuery('.top-bar-wrap').toggleClass('top-nav-dark-skin');
        });


        if (jQuery("input[name='thf_options[top-nav-area-1]']").val() == 'components' || jQuery("input[name='thf_options[top-nav-area-1]']").val() == 'components') {
            jQuery('.top-nav-components-wrapper').show();
        }

        jQuery("input[name='thf_options[top-nav-area-2]']").change(function () {
            selected_val = jQuery(this).val();
            if (selected_val == 'components') {
                jQuery("input[name='thf_options[top-nav-area-1]'][value='components']").removeAttr('checked');
                jQuery('#top-nav-components-1').hide();
            } else {
                if (selected_val == 'menu') {
                    jQuery("input[name='thf_options[top-nav-area-1]'][value='menu']").removeAttr('checked');
                    jQuery('#top-nav-menu-1').hide();
                }

                if (jQuery("input[name='thf_options[top-nav-area-1]']:checked").val() != 'components') {
                    jQuery('.top-nav-components-wrapper').hide();
                }
            }
        });

        jQuery("input[name='thf_options[top-nav-area-1]']").change(function () {
            selected_val = jQuery(this).val();
            if (selected_val == 'components') {
                jQuery("input[name='thf_options[top-nav-area-2]'][value='components']").removeAttr('checked');
                jQuery('#top-nav-components-2').hide();
            } else {

                if (selected_val == 'menu') {
                    jQuery("input[name='thf_options[top-nav-area-2]'][value='menu']").removeAttr('checked');
                    jQuery('#top-nav-menu-2').hide();
                }

                if (jQuery("input[name='thf_options[top-nav-area-2]']:checked").val() != 'components') {
                    jQuery('.top-nav-components-wrapper').hide();
                }
            }
        });

        jQuery("input[name='thf_options[main_nav]']").change(function () {
            $header_preview_area.toggleClass('main-nav-disabled');
        });

        jQuery("input[name='thf_options[main_nav_position]']").change(function () {
            $header_preview_area.toggleClass('main-nav-above');
        });

        jQuery("input[name='thf_options[main_nav_layout]']").change(function () {
            jQuery('.header-main-menu-wrap').toggleClass('main-nav-full');
        });

        jQuery("input[name='thf_options[main_nav_dark]']").change(function () {
        ...
        var lastScrollTop     = 0,
            $topSaveButton    = jQuery('.thf-panel-content'),
            $bottomSaveButton = jQuery('.thf-footer .thf-save-button');
    ...
    $doc.on('click', '#thf-builder-wrapper .thf-options a', function () {
    ...
    /* Blocks Color Picker */
    var thfBlocksColorsOptions = {
        change: function (event, ui) {
            var newColor = ui.color.toString();
            jQuery(this).closest('.block-item').find('.thf-block-head').attr('style', 'background-color: ' + newColor).removeClass('block-head-light block-head-dark').addClass('block-head-' + getContrastColor(newColor));
        },
        clear : function () {
            jQuery(this).closest('.block-item').find('.thf-block-head').attr('style', '').removeClass('block-head-light block-head-dark');
        }
    };

    if (jQuery().wpColorPicker) {
        jQuery('.thfBlocksColor').wpColorPicker(thfBlocksColorsOptions);
    }


    /* Enable/Disable the Builder For the page */
    jQuery('#thf-page-builder-button').click(function () {

        $thfBody.toggleClass('builder-is-active');
        jQuery('#thf-page-builder').attr('style', '');

        var tabsHeight = jQuery('.thf-panel-tabs').outerHeight();
        jQuery('.thf-panel-content').css({minHeight: tabsHeight});
        ...
        if ($pageBuilderButton.hasClass('builder_active')) {
            ...
            jQuery('#thf_builder_active').val('');
            pagePosition = $thfBody.scrollTop();
            ...
            jQuery('#thf_builder_active').val('yes');

            jQuery('.tabs-wrap').hide();
            jQuery('.thf-panel-tabs ul li').removeClass('active');

            jQuery('.thf-panel-tabs ul li:first').addClass('active').show();
    ...
    if (jQuery('#thf_builder_active').val() == 'yes') {
        $thfBody.addClass('builder-is-active');
    }

    /* Add a new block button */
    $doc.on('click', '.thf-add-new-block', function () {

        var $thisButton  = jQuery(this),
            $loadSpinner = $thisButton.closest('.thf-builder-container').find('.thf-loading-container').addClass('is-animated').show(),
            sectionID    = $thisButton.data('section'),
            blockID      = thf_block_id;
        uniqueID = sectionID + '-' + blockID;

        jQuery.post(
            ajaxurl,
            {
                action    : 'thf_get_builder_blocks',
                block_id  : blockID,
            ...
            function (data) {
                ...
                content.find('.tabs_cats').sortable({placeholder: 'thf-state-highlight'});

                content.appendTo('#cat_sortable_' + sectionID);

                content.find('.thf-img-path').each(function () {
                    thf_image_uploader_trigger(jQuery(this));
                });

                thf_builder_dragdrop();

                jQuery('.thf-builder-blocks-wrapper').sortable();

                var addedBlock = jQuery('#listItem_' + uniqueID);

                //addedBlock.hide().fadeIn();
                addedBlock.find('.thf-builder-content-area, .toggle-close').show();
                addedBlock.find('.toggle-open').hide();
                addedBlock.find('.thfBlocksColor').wpColorPicker(thfBlocksColorsOptions);

                $thfBody.addClass('has-overlay');

                // Switch Button ----------
                var $blockSwitches = addedBlock.find('.thf-js-switch');
    ...
    $widgetsCustomizer = jQuery('#thf-sidebars-customize');
    $customWidgetsHolder = jQuery('#custom-widgtes-settings');

    $doc.on('click', '.thf-manage-widgets', function () {

        var $thisButton     = jQuery(this),
            sectionID       = $thisButton.data('widgets'),
            $sidebarOptions = jQuery('#' + sectionID + '-sidebar-options'),
            $widgetsToggle  = $sidebarOptions.find('.thf-toggle-option');
    ...
    $widgetsCustomizer.find('.thf-toggle-option').change(function () {
        $customWidgetsHolder.toggle();
    });


    /* Add a new section button */
    $doc.on('click', '.thf-add-new-section', function () {

        var $thisButton  = jQuery(this).hide(),
            sectionID    = $thisButton.data('sections'),
            postID       = $thisButton.data('post'),
            $loadSpinner = $thisButton.closest('.thf-add-new-section-wrapper').find('.thf-loading-container').addClass('is-animated').show();


        jQuery.post(
            ajaxurl,
            {
                action    : 'thf_get_builder_section',
                section_id: sectionID,
                post_id   : postID
            },
            function (data) {
                var sectionUnique = 'section-' + Math.floor(Math.random() * 10000),
                    content       = data.replace(/thfxyz20/g, sectionUnique);

                content = jQuery(content);
                content.appendTo('#thf-builder-wrapper');
                ...
                content.find('.thf-builder-content-area').show();

                var $blockSwitches = content.find('.thf-js-switch');
                $blockSwitches.each(function () {
                    new Switchery(this, {color: brandColor});
                ...
                content.find('.thfColorSelector').wpColorPicker(thfBlocksColorsOptions);
                content.find('.sections-sidebars-options').appendTo('#section-sidebar-options').find('.thf-toggle-option').change(function () {
                ...
                jQuery('#widgets-right').append('\
					<div id="wrap-thfpost-' + postID + '-' + sectionUnique + '"class="widgets-holder-wrap">\
						<div id="thfpost-' + postID + '-' + sectionUnique + '" class="widgets-sortables">\
    ...
    /* Section Sidebar */
    $doc.on('click', '.thf-section-sidebar-options a', function () {

        var $thisElement = jQuery(this),
            $theSection  = $thisElement.closest('.thf-builder-container');
    ...
    $doc.keyup(function (event) {
        if (event.which == '27' && ($thfBody.hasClass('builder-is-active') || $thfBody.hasClass('thfplugin_page_thf-one-click-demo-import'))) {

            if ($thfBody.hasClass('force-refresh')) {
                location.reload();
            }

            $thfBody.removeClass('has-overlay');
            jQuery('.thf-popup-window').hide();
        }
    });

    /* Edit Block Done Button */
    $doc.on('click', '.builder-is-active #thf-page-overlay, #thf-page-overlay.is-notice-dismissible, .thfplugin_page_thf-one-click-demo-import #thf-page-overlay, .thf-popup-window .close, .thf-edit-block-done', function () {
    ...
    $doc.on('click', '.toggle-section', function () {
        var $thisElement = jQuery(this).closest('.thf-builder-container');
        $thisElement.find('.thf-builder-section-inner').slideToggle('fast');
        $thisElement.toggleClass('thf-section-open');
        return false;
    });
    ...
    $doc.on('click', '.edit-block-icon', function () {
        ...
        $thfBody.addClass('has-overlay');
        $thisElement.find('> .thf-builder-content-area').fadeIn('fast');
        return false;
    });
    ...
    $doc.on('click', '.blocks-settings-tabs a', function () {
        var $thisButtonTab = jQuery(this),
            $blockContent  = $thisButtonTab.closest('.thf-builder-content-area'),
    ...
    $doc.on('click', '#thf_featured_posts_style a', function () {
    ...
    jQuery('.thf-img-path').each(function () {
        thf_image_uploader_trigger(jQuery(this));
    });


    /* FONTS */
    jQuery('.thf-select-font').fontselect();


    /* CHECKBOXES */
    var checkInputs = Array.prototype.slice.call(document.querySelectorAll('.thf-js-switch'));
    ...
    if (jQuery('li.menu-top.toplevel_page_thf-plugin-options .thf-plugin-update').length) {
        jQuery('li.menu-top.toplevel_page_thf-plugin-options .wp-menu-name').append(' <span class="update-plugins"><span class="update-count">' + thfLang.update + '</span></span>');
    }


    /* Widgets
	------------------------------------------------------------------------------------------ */
    if ($thfBody.hasClass('widgets-php') || $thfBody.hasClass('nav-menus-php') || $thfBody.hasClass('post-type-page')) {
        $doc.ajaxComplete(function () {
            jQuery('.thfColorSelector').wpColorPicker();
        });
    }

    /* Widget Tabs Sortable  */
    jQuery('.tab-sortable').each(function () {
        thf_sortable_tabs_trigger(jQuery(this));
    });

    /* Widget Posts order option  */
    jQuery('.thf-posts-order-option').each(function () {
        thf_widget_posts_order(jQuery(this));
    });
    ...
    $doc.on('widget-added', function (event, widgetContainer) {
        ...
        thf_sortable_tabs_trigger($thisTabs);

        // ------
        var $thisOption = widgetContainer.find('.thf-posts-order-option');
        thf_widget_posts_order($thisOption);
    });
    ...
    /* DISMISS NOTICES
	------------------------------------------------------------------------------------------ */
    $doc.on('click', '.thf-notice .notice-dismiss', function () {

        jQuery('#thf-page-overlay').hide();

        jQuery.ajax({
            url : ajaxurl,
            type: 'post',
            data: {
                pointer: jQuery(this).closest('.thf-notice').attr('id'),
                action : 'dismiss-wp-pointer',
            },
        });
    });
    ...
    /* SAVE THEME SETTINGS
	------------------------------------------------------------------------------------------ */
    var $saveAlert = jQuery('#thf-saving-settings');

    jQuery('#thf_form').submit(function () {
        ...
        jQuery('form#thf_form input, form#thf_form textarea, form#thf_form select').each(function () {
            if (!jQuery(this).val()) {
        ...
        jQuery('form#thf_form input:disabled, form#thf_form textarea:disabled, form#thf_form select:disabled').attr('disabled', false);
    ...
    /* SAVE SETTINGS ALERT */
    $saveAlert.fadeOut();
    jQuery('.thf-save-button').click(function () {
        $saveAlert.fadeIn();
    });
    ...
    /* THEME PANEL
	------------------------------------------------------------------------------------------ */
    jQuery('.thf-panel, .thf-notice').css({'opacity': 1, 'visibility': 'visible'});

    var tabsHeight = jQuery('.thf-panel-tabs').outerHeight();
    jQuery('.tabs-wrap').hide();
    jQuery('.thf-panel-content').css({minHeight: tabsHeight});

    jQuery('li.thf-tabs:not(.thf-not-tab) > a').click(function (e) {
        e.preventDefault();
        var activeTab = jQuery(this).attr('href');
        var parent = jQuery(this).parents('ul').attr('data-parent') || null;
        var prefix = "thf-options-tab-";

        jQuery('.thf-tabs').removeClass('active');
        jQuery('.thf-tabs' + activeTab.replace('#', '.')).addClass('active');
        ...
        if (typeof parent !== 'undefined' && parent !== null && parent !== '') {
            ...
            parent = tabs[0];
            tabs.splice(0, 1); // Remove first key
            ...
            jQuery.each(tabs, function (index, value) {
                ...
                jQuery(".thf-tabs.has-child." + prefix + parent + "-" + tab).addClass('active');
            });

        } else if (activeTab !== "") {
            parent = activeTab.replace("#", "");
            parent = parent.replace(prefix, "");
            parent = parent.split('-');
            parent = parent[0];
        }

        jQuery('.thf-panel-tabs .' + prefix + parent).addClass('active');
    ...
    if (jQuery(currentTab).parent('#thf_form').length) {
        jQuery('a[href="' + currentTab + '"]').click();
        ...
        jQuery('.thf-panel-tabs ul li.thf-tabs:not(.thf-not-tab):first > a').click();
    }
    ...
    $doc.on('click', '.del-item', function () {
        ...
        if ($thisButton.hasClass('del-section')) {
            var widgets = $thisButton.closest('.parent-item').find('.thf-manage-widgets').data('widgets');
    ...
    $doc.on('click', '.del-img', function () {
        ...
        $img.fadeOut('fast', function () {
            $img.hide();
            $img.closest('.option-item').find('.thf-img-path').attr('value', '');
        });
    });
    ...
    jQuery('#breaking_news_button').click(function () {
        var customlink = jQuery('#custom_link').val(),
            customtext = thfHTMLspecialchars(jQuery('#custom_text').val());

        if (customtext.length > 0 && customlink.length > 0) {
            jQuery('#breaking_custom_error-item').slideUp();
            jQuery('#customList').append('\
				<li class="parent-item">\
					<div class="thf-block-head">\
						<a href="' + customlink + '" target="_blank">' + customtext + '</a>\
						<input name="thf_options[breaking_custom][' + customnext + '][link]" type="hidden" value="' + customlink + '" />\
						<input name="thf_options[breaking_custom][' + customnext + '][text]" type="hidden" value="' + customtext + '" />\
    ...
    jQuery('#add_highlights_button').click(function () {
        var customtext = thfHTMLspecialchars(jQuery('#custom_text').val());
        if (customtext.length > 0) {
            ...
            jQuery('#customList').append('\
				<li class="parent-item">\
					<div class="thf-block-head">\
						' + customtext + '\
						<input name="thf_highlights_text[' + customnext + ']" type="hidden" value="' + customtext + '" />\
						<a class="del-item dashicons dashicons-trash"></a>\
					</div>\
				</li>\
			');
    ...
    jQuery('#add_source_button').click(function () {
        var source_name = thfHTMLspecialchars(jQuery('#source_name').val()),
        ...
        if (source_name.length > 0) {
            ...
            if (source_link.length > 0) {
                source_code += '\
							...
							<input name="thf_source[' + source_next + '][url]" type="hidden" value="' + source_link + '" />\
						';
            }
            else {
            ...
            source_code += '\
						<input name="thf_source[' + source_next + '][text]" type="hidden" value="' + source_name + '" />\
						<a class="del-item dashicons dashicons-trash"></a>\
					</div>\
				</li>\
			';
    ...
    jQuery('#add_via_button').click(function () {
        var via_name = thfHTMLspecialchars(jQuery('#via_name').val()),
        ...
        if (via_name.length > 0) {
            ...
            if (via_link.length > 0) {
                via_code += '\
							...
							<input name="thf_via[' + via_next + '][url]" type="hidden" value="' + via_link + '" />\
						';
            }
            else {
                via_code += via_name;
            ...
            via_code += '\
						<input name="thf_via[' + via_next + '][text]" type="hidden" value="' + via_name + '" />\
						<a class="del-item dashicons dashicons-trash"></a>\
					</div>\
				</li>\
			';
        ...
        if (SidebarName.length > 0) {
            ...
            jQuery('#sidebarsList').append('\
				<li class="parent-item">\
					<div class="thf-block-head">\
						' + SidebarName + '\
						<input id="thf_sidebars" name="thf_options[sidebars][]" type="hidden" value="' + SidebarName + '" />\
    ...
    /* VISUAL OPTIONS
	------------------------------------------------------------------------------------------ */
    jQuery('ul.thf-options').each(function (index) {
        jQuery(this).find('input:checked').parent().addClass('selected');
    });

    $doc.on('click', 'ul.thf-options a', function () {
        var $thisBlock = jQuery(this),
            blockID    = $thisBlock.closest('ul.thf-options').attr('id');
    ...
    if ($thfBody.hasClass('taxonomy-category')) {
    ...
    if (selected_item == 'template-masonry.php') {
        jQuery('#thf-page-template-categories').show();
    }
    else if (selected_item == 'template-authors.php') {
        jQuery('#thf-page-template-authors').show();
    }

    jQuery("select[name='page_template']").change(function () {
        var selected_item = jQuery("select[name='page_template'] option:selected").val();
        jQuery('.thf-page-templates-options').hide();
        if (selected_item == 'template-masonry.php') {
            jQuery('#thf-page-template-categories').show();
        }
        else if (selected_item == 'template-authors.php') {
    ...
    jQuery('.predefined-skins-options select').change(function () {
        var skin        = jQuery(this).find('option:selected').val(),
            skin_colors = thf_skins[skin];

        jQuery('#thf-options-tab-styling').find('.thfColorSelector').val('');
        jQuery('#thf-options-tab-styling').find('.wp-color-result').attr('style', '');
...
------------------------------------------------------------------------------------------ */
function thf_sortable_tabs_trigger($thisTabs) {

    $thisTabs.sortable({
        placeholder: 'thf-state-highlight',
...
/* Fire Sortable on the Widgets Tabs
------------------------------------------------------------------------------------------ */
function thf_widget_posts_order($thisElement) {

    $thisElement.change(function () {

        var $thisElement      = jQuery(this),
            jetPackOption     = $thisElement.closest('.widget-content').find('.thf-jetpack-posts-order-option'),
            relatedOption     = $thisElement.closest('.widget-content').find('.thf-related-posts-order-option'),
            NoneCustomOptions = $thisElement.closest('.widget-content').find('.thf-non-custom-posts-order-option'),
...
/* IMAGE UPLOADER PREVIEW
------------------------------------------------------------------------------------------ */
function thf_image_uploader_trigger($thisElement) {
    ...
    if ($thisElement.hasClass('thf-background-path')) {
        thisElementID = thisElementID.replace('-img', '');
        uploaderTypeStyles = true;
    }

    thf_set_uploader(thisElementID, uploaderTypeStyles);
}


/* IMAGE UPLOADER FUNCTIONS
------------------------------------------------------------------------------------------ */
function thf_builder_dragdrop() {

    jQuery('#thf-builder-wrapper').sortable({
        placeholder: 'thf-state-highlight thf-state-sections',
        activate   : function (event, ui) {

            var $sortableWrap = ui.item,
                outerHeight   = $sortableWrap.outerHeight() + 40;

            jQuery('.thf-state-sections').css('min-height', outerHeight);
        },
    });

    jQuery('.tabs_cats').sortable({placeholder: 'thf-state-highlight'});
    ...
    jQuery('.block-item').draggable({
        ...
        cursor           : 'move',
        zIndex           : 100,
        connectToSortable: '.thf-builder-blocks-wrapper',
        revert           : true,
        revertDuration   : 0,
    ...
    jQuery('.thf-builder-blocks-wrapper').sortable({
        ...
        receive: function (event, ui) {
            ...
            ui.item.find('[name^=thf_home_cats]').each(function () {
                var newName = jQuery(this).attr('name').replace(/thf_home_cats\[(\w+)\]/g, 'thf_home_cats\[' + sectionID + ']');
                jQuery(this).attr('name', newName);
            });
        },

        activate: function (event, ui) {
            jQuery('.thf-builder-blocks-wrapper').css('min-height', '65px');
            var $sortableWrap = ui.item.closest('.thf-builder-blocks-wrapper'),
                outerHeight   = ($sortableWrap.outerHeight() > 0) ? $sortableWrap.outerHeight() + 40 : '65px';

            $sortableWrap.css('min-height', outerHeight);
            jQuery('.thf-builder-container').addClass('thf-block-hover');
        },

        deactivate: function () {
            jQuery('.thf-builder-container').removeClass('thf-block-hover');
            jQuery('.thf-builder-blocks-wrapper').css('min-height', '');
        },
    }).sortable('option', 'connectWith', '.thf-builder-container');
...
/* IMAGE UPLOADER FUNCTIONS
------------------------------------------------------------------------------------------ */
function thf_set_uploader(field, styling) {
    var thf_bg_uploader;

    $doc.on('click', '#upload_' + field + '_button', function (event) {

        event.preventDefault();
        thf_bg_uploader = wp.media.frames.thf_bg_uploader = wp.media({
            title   : 'Choose Image',
            library : {type: 'image'},
            button  : {text: 'Select'},
            multiple: false
        ...
        thf_bg_uploader.on('select', function () {
            var selection = thf_bg_uploader.state().get('selection');
...
function thf_color_picker() {
    ...
    jQuery('.thfColorSelector').each(function () {
        ...
        jQuery('<div class="thf-alpha-container"><div class="slider-alpha"></div><div class="transparency"></div></div>').appendTo($control.parents('.wp-picker-container'));
...
function thfHTMLspecialchars(text) {
    var map = {
        '&': '&amp;',
        '<': '&lt;',
        '>': '&gt;',
...
(function () {
    ...
    require.register("switchery", function (exports, module) {
        ...
        var defaults = {color: "#64bd63", secondaryColor: "#dfdfdf", jackColor: "#fff", className: "switchery", disabled: false, disabledOpacity: .5, speed: "0.4s", size: "default"};
	...
	*/
        ...
        function createPopup($button) {
            ...
            $control.html('<a data-direction="back" href="#"><span class="dashicons dashicons-arrow-left-alt2"></span></a> ' +
                '<input type="text" class="" placeholder="' + thfLang.search + '" />' +
...
!function (e) {
    ...
    !function (t) {
        var o = "function" == typeof define && define.amd, a = "undefined" != typeof module && module.exports, n = "https:" == document.location.protocol ? "https:" : "http:",
        ...
        var t, o = "mCustomScrollbar", a = "mCS", n = ".mCustomScrollbar", i = {
        ...
        }, r     = 0, l = {}, s = window.attachEvent && !window.addEventListener ? 1 : 0, c = !1, d = [
            "mCSB_dragger_onDrag",
            ...
            var t                                                                                                                = e(this), n                                                                                                   = t.data(a), i = n.opt, r = i.autoExpandScrollbar ? " " + d[1] + "_expand" : "", l = [
                ...
                ], s                                                                                                             = "yx" === i.axis ? "mCSB_vertical_horizontal" : "x" === i.axis ? "mCSB_horizontal" : "mCSB_vertical", c = "yx" === i.axis ? l[0] + l[1] : "x" === i.axis ? l[1] : l[0],
                u = "yx" === i.axis ? "<div id='mCSB_" + n.idx + "_container_wrapper' class='mCSB_container_wrapper' />" : "", f = i.autoHideScrollbar ? " " + d[6] : "",
            ...
            var o = [
                t[0].scrollWidth, Math.max.apply(Math, t.children().map(function () {
            ...
            var t = e(this), o = t.data(a), n = o.opt, i = e(".mCSB_" + o.idx + "_scrollbar:first"), r = oe(n.scrollButtons.tabindex) ? "tabindex='" + n.scrollButtons.tabindex + "'" : "", l = [
                "<a href='#' class='" + d[13] + "' " + r + " />",
                "<a href='#' class='" + d[14] + "' " + r + " />",
                "<a href='#' class='" + d[15] + "' " + r + " />",
                "<a href='#' class='" + d[16] + "' " + r + " />"
            ...
            var t                                                                                               = e(this), o                                                                                  = t.data(a), n = e("#mCSB_" + o.idx), i                                          = e("#mCSB_" + o.idx + "_container"),
                r = [e("#mCSB_" + o.idx + "_dragger_vertical"), e("#mCSB_" + o.idx + "_dragger_horizontal")], l = [n.height() / i.outerHeight(!1), n.width() / i.outerWidth(!1)],
            ...
            var t                                                                                               = e(this), o                                                                                  = t.data(a), n = e("#mCSB_" + o.idx), i                                          = e("#mCSB_" + o.idx + "_container"),
                r = [e("#mCSB_" + o.idx + "_dragger_vertical"), e("#mCSB_" + o.idx + "_dragger_horizontal")], l = [i.outerHeight(!1) - n.height(), i.outerWidth(!1) - n.width()],
            ...
            var t = e(this), o = t.data(a), n = e("#mCSB_" + o.idx), i = e("#mCSB_" + o.idx + "_container"), r = null == o.overflowed ? i.height() : i.outerHeight(!1),
            ...
            var o                                                                                                   = e(this), n = o.data(a), i = n.opt, r = e("#mCSB_" + n.idx + "_container_wrapper"), l            = r.length ? r : e("#mCSB_" + n.idx + "_container"),
                s = [e("#mCSB_" + n.idx + "_scrollbar_vertical"), e("#mCSB_" + n.idx + "_scrollbar_horizontal")], c = [s[0].find(".mCSB_dragger"), s[1].find(".mCSB_dragger")];
            "x" !== i.axis && (n.overflowed[0] && !t ? (s[0].add(c[0]).add(s[0].children("a")).css("display", "block"), l.removeClass(d[8] + " " + d[10])) : (i.alwaysShowScrollbar ? (2 !== i.alwaysShowScrollbar && c[0].css("display", "none"), l.removeClass(d[10])) : (s[0].css("display", "none"), l.addClass(d[10])), l.addClass(d[8]))), "y" !== i.axis && (n.overflowed[1] && !t ? (s[1].add(c[1]).add(s[1].children("a")).css("display", "block"), l.removeClass(d[9] + " " + d[11])) : (i.alwaysShowScrollbar ? (2 !== i.alwaysShowScrollbar && c[1].css("display", "none"), l.removeClass(d[11])) : (s[1].css("display", "none"), l.addClass(d[11])), l.addClass(d[9]))), n.overflowed[0] || n.overflowed[1] ? o.removeClass(d[5]) : o.addClass(d[5])
        }, O     = function (t) {
            var o = t.type, a = t.target.ownerDocument !== document && null !== frameElement ? [e(frameElement).offset().top, e(frameElement).offset().left] : null,
            ...
            var o, n, i, r                                                                 = e(this), l = r.data(a), d                                     = l.opt, u = a + "_" + l.idx, f = ["mCSB_" + l.idx + "_dragger_vertical", "mCSB_" + l.idx + "_dragger_horizontal"],
                h = e("#mCSB_" + l.idx + "_container"), m = e("#" + f[0] + ",#" + f[1]), p = d.advanced.releaseDraggableSelectors ? m.add(e(d.advanced.releaseDraggableSelectors)) : m,
            ...
            function n(e) {
                if (te(e) && !c && !O(e)[2] && (T.documentTouchScroll || e.preventDefault(), e.stopImmediatePropagation(), (!C || b) && d)) {
                    ...
                    r || h ? (U || e.preventDefault(), b = 1) : (C = 1, y.addClass("mCS_touch_action")), U && e.preventDefault(), w = "yx" === T.axis ? [u - o, f - a] : "x" === T.axis ? [
                        null,
                        f - a
                    ] : [u - o, null], I[0].idleTimer = 250, B.overflowed[0] && s(w[0], R, n, "y", "all", !0), B.overflowed[1] && s(w[1], R, n, "x", L, !0)
                }
            }
            ...
            var d, u, f, h, m, p, g, v, x, _, w, S, b, C, y = e(this), B = y.data(a), T = B.opt, k = a + "_" + B.idx, M = e("#mCSB_" + B.idx), I = e("#mCSB_" + B.idx + "_container"),
                D                                                                                                                                = [e("#mCSB_" + B.idx + "_dragger_vertical"), e("#mCSB_" + B.idx + "_dragger_horizontal")], E = [], W                          = [], R = 0,
                L                                                                                                                                = "yx" === T.axis ? "none" : "all", z = [], P = I.find("iframe"), H = [
            ...
            function t(t, a) {
                if (Q(o), !z(o, t.target)) {
                    ...
                    if ("x" === i.axis || "x" === i.mouseWheel.axis) {
                        var u                                                                 = "x", f = [Math.round(r * n.scrollRatio.x), parseInt(i.mouseWheel.scrollAmount)],
                            h                                                                 = "auto" !== i.mouseWheel.scrollAmount ? f[1] : f[0] >= l.width() ? .9 * l.width() : f[0],
                            m = Math.abs(e("#mCSB_" + n.idx + "_container")[0].offsetLeft), p = c[1][0].offsetLeft,
                            g                                                                 = c[1].parent().width() - c[1].width(), v                       = "y" === i.mouseWheel.axis ? t.deltaY || a : t.deltaX;
                    } else {
                        var u = "y", f = [
            ...
            if (e(this).data(a)) {
                var o = e(this), n = o.data(a), i = n.opt, r = a + "_" + n.idx, l = e("#mCSB_" + n.idx), c = [e("#mCSB_" + n.idx + "_dragger_vertical"), e("#mCSB_" + n.idx + "_dragger_horizontal")],
            ...
            r.bind("focusin." + i, function () {
                ...
                o.is(n.advanced.autoScrollOnFocus) && (Q(t), clearTimeout(t[0]._focusTimeout), t[0]._focusTimer = a ? (i + 17) * a : 0, t[0]._focusTimeout = setTimeout(function () {
                    ...
                    "x" === n.axis || s[0] || G(t, e[0].toString(), {dir: "y", scrollEasing: "mcsEaseInOut", overwrite: c, dur: i}), "y" === n.axis || s[1] || G(t, e[1].toString(), {
                        dir         : "x",
                        scrollEasing: "mcsEaseInOut",
                        overwrite   : c,
                        dur         : i
            ...
            var o = e(this), n = o.data(a), i = n.opt, r = n.sequential, l = a + "_" + n.idx, s = e("#mCSB_" + n.idx), c = e("#mCSB_" + n.idx + "_container"), d = c.parent(),
                u                                                                                                                                                = "input,textarea,select,datalist,keygen,[contenteditable='true']", f                                                                          = c.find("iframe"), h = ["blur." + l + " keydown." + l + " keyup." + l];
            ...
            function l(e) {
                ...
                var o                                                                                                                 = "stepped" !== f.type, a = r ? r : e ? o ? p / 1.5 : g : 1e3 / 60, n = e ? o ? 7.5 : 40 : 2.5, s = [Math.abs(h[0].offsetTop), Math.abs(h[0].offsetLeft)],
                    d                                                                                                                 = [c.scrollRatio.y > 10 ? 10 : c.scrollRatio.y, c.scrollRatio.x > 10 ? 10 : c.scrollRatio.x], m = "x" === f.dir[0] ? s[1] + f.dir[1] * (d[1] * n) : s[0] + f.dir[1] * (d[0] * n),
                    v = "x" === f.dir[0] ? s[1] + f.dir[1] * parseInt(f.scrollAmount) : s[0] + f.dir[1] * parseInt(f.scrollAmount), x = "auto" !== f.scrollAmount ? v : m,
            ...
            var c = t.data(a), u = c.opt, f = c.sequential, h = e("#mCSB_" + c.idx + "_container"), m = "stepped" === f.type, p = u.scrollInertia < 26 ? 26 : u.scrollInertia,
            ...
            return "function" == typeof t && (t = t()), t instanceof Array ? n = t.length > 1 ? [t[0], t[1]] : "x" === o.axis ? [null, t[0]] : [
                t[0],
                null
            ] : (n[0] = t.y ? t.y : t.x || "x" === o.axis ? null : t, n[1] = t.x ? t.x : t.y || "y" === o.axis ? null : t), "function" == typeof n[0] && (n[0] = n[0]()), "function" == typeof n[1] && (n[1] = n[1]()), n
        }, X     = function (t, o) {
            ...
            var s                                                                                                                       = t.data(a), c = s.opt,
                d                                                                                                                       = {trigger: "internal", dir: "y", scrollEasing: "mcsEaseOut", drag: !1, dur: c.scrollInertia, overwrite: "all", callbacks: !0, onStart: !0, onUpdate: !0, onComplete: !0},
                n = e.extend(d, n), u = [n.dur, n.drag ? 0 : n.dur], f = e("#mCSB_" + s.idx), h = e("#mCSB_" + s.idx + "_container"), m = h.parent(),
            ...
            if (s.trigger = n.trigger, 0 === m.scrollTop() && 0 === m.scrollLeft() || (e(".mCSB_" + s.idx + "_scrollbar").css("visibility", "visible"), m.scrollTop(0).scrollLeft(0)), "_resetY" !== o || s.contentReset.y || (i("onOverflowYNone") && c.callbacks.onOverflowYNone.call(t[0]), s.contentReset.y = 1), "_resetX" !== o || s.contentReset.x || (i("onOverflowXNone") && c.callbacks.onOverflowXNone.call(t[0]), s.contentReset.x = 1), "_resetY" !== o && "_resetX" !== o) {
                ...
                switch (n.dir) {
                    case"x":
                        var x                                                                                                      = e("#mCSB_" + s.idx + "_dragger_horizontal"), _ = "left", w                                         = h[0].offsetLeft, S = [f.width() - h.outerWidth(!1), x.parent().width() - x.width()],
                            b = [o, 0 === o ? 0 : o / s.scrollRatio.x], y = p[1], B = g[1], T = y > 0 ? y / s.scrollRatio.x : 0, k = B > 0 ? B / s.scrollRatio.x : 0;
                        break;
                    case"y":
                        var x                                                                                                      = e("#mCSB_" + s.idx + "_dragger_vertical"), _ = "top", w = h[0].offsetTop, S = [f.height() - h.outerHeight(!1), x.parent().height() - x.height()],
                            b = [o, 0 === o ? 0 : o / s.scrollRatio.y], y = p[0], B = g[0], T = y > 0 ? y / s.scrollRatio.y : 0, k = B > 0 ? B / s.scrollRatio.y : 0
                }
                b[1] < 0 || 0 === b[0] && 0 === b[1] ? b = [0, 0] : b[1] >= S[1] ? b = [
                    S[0],
                    S[1]
                ] : b[0] = -b[0], t[0].mcs || (l(), i("onInit") && c.callbacks.onInit.call(t[0])), clearTimeout(h[0].onCompleteTimeout), J(x[0], _, Math.round(b[1]), u[1], n.scrollEasing), !s.tweenRunning && (0 === w && b[0] >= 0 || w === S[0] && b[0] <= S[0]) || J(h[0], _, Math.round(b[0]), u[0], n.scrollEasing, n.overwrite, {
                    onStart   : function () {
            ...
            function e() {
                ...
                for (var t = 0; t < e.length; t++) if (e[t] + "Hidden" in document) {
                    return e[t] + "Hidden";
                }
                return null
            }
        ...
        e.fn[o] = function (t) {
            ...
            e(n)[o](), e.extend(e.expr[":"], {
                mcsInView     : e.expr[":"].mcsInView || function (t) {
                    ...
                    if (c.length) {
                        return n = [s.outerHeight(!1), s.outerWidth(!1)], r = [c[0].offsetTop + ae(s)[0], c[0].offsetLeft + ae(s)[1]], i = [
