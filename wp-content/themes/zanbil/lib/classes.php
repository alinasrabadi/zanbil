...
class ZANBIL_Config {
	...
	public static function setVariables($vars){
		if ( is_null(self::$instance) ){
			self::$instance = new ZANBIL_Config();
...
if ( !class_exists('ZANBIL_Widget') ):

abstract class ZANBIL_Widget extends WP_Widget{
	...
	protected function init(){
		...
		$this->base_tpl_path = apply_filters('zanbil_widget_template_base', $this->base_path.'/widgets/'.$this->id_base.'/tmpl');
		$this->override_tpl_path = apply_filters('zanbil_widget_override_base', get_template_directory().'/widgets/'.$this->id_base);
	}
	...
	public function zanbil_trim_words( $text, $num_words = 30, $more = null ) {
	...
	public function widget_template_select( $field_name, $opts = array(), $field_value = null ){
		...
		if ( array_key_exists('ov', $templates) && count($templates['ov']) ){
			$select_html .= '<optgroup label="'. esc_attr__('Override by Theme', 'zanbil') .'">';
		...
		if ( array_key_exists('df', $templates) && count($templates['df']) ){
			$select_html .= '<optgroup label="'. esc_attr__('Default Template', 'zanbil') .'">';
	...
	public function form( $instance ){
		...
		?>
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id('title') ); ?>"><?php esc_html_e('Title', 'zanbil')?></label>
			<br />
			<input class="widefat"
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id('widget_template') ); ?>"><?php esc_html_e("Template", 'zanbil')?></label>
			<br/>
